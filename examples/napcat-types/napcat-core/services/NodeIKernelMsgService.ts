import { ElementType, MessageElement, Peer, RawMessage, FileElement, SendMessageElement, AvRecordElement, TofuRecordElement } from '../types';
import { NodeIKernelMsgListener } from '../listeners/NodeIKernelMsgListener';
import { GeneralCallResult } from '../services/common';
import { MsgReqType, QueryMsgsParams, TmpChatInfoApi, MsgTypeFilter, MsgIdentity, SgrpStreamParams, GrayTipJsonInfo, ForwardFileInfo, LocalGrayTipInfo, TokenInfo, BackGroundInfo } from '../types/msg';
export interface NodeIKernelMsgService {
    buildMultiForwardMsg(req: {
        srcMsgIds: Array<string>;
        srcContact: Peer;
    ...
    }, msgId: string, count: number, queryOrder: boolean): Promise<GeneralCallResult & {
        msgList: RawMessage[];
    }>;
    getMsgsIncludeSelf(peer: Peer, msgId: string, count: number, queryOrder: boolean): Promise<GeneralCallResult & {
        msgList: RawMessage[];
    }>;
    getMsgsWithMsgTimeAndClientSeqForC2C(peer: Peer, arg2: string, arg3: string, arg4: number, arg5: boolean, arg6: boolean, arg7: boolean): Promise<GeneralCallResult & {
        msgList: RawMessage[];
    }>;
    ...
    getMsgsBySeqList(peer: Peer, seqList: string[]): Promise<GeneralCallResult & {
        msgList: RawMessage[];
    }>;
    getMsgsExt(msgReq: MsgReqType): Promise<GeneralCallResult & {
        msgList: RawMessage[];
    }>;
    getSingleMsg(Peer: Peer, msgSeq: string): Promise<GeneralCallResult & {
        msgList: RawMessage[];
    }>;
    getSourceOfReplyMsg(peer: Peer, msgId: string, sourceSeq: string): Promise<GeneralCallResult & {
        msgList: RawMessage[];
    }>;
    getSourceOfReplyMsgV2(peer: Peer, rootMsgId: string, replyMsgId: string): Promise<GeneralCallResult & {
        msgList: RawMessage[];
    }>;
    getMsgByClientSeqAndTime(peer: Peer, clientSeq: string, time: string): Promise<GeneralCallResult & {
        msgList: RawMessage[];
    }>;
    getSourceOfReplyMsgByClientSeqAndTime(peer: Peer, clientSeq: string, time: string, replyMsgId: string): Promise<GeneralCallResult & {
        msgList: RawMessage[];
    }>;
    ...
    getMsgWithAbstractByFilterParam(arg1: Peer, arg2: string, arg3: string, arg4: number, arg5: MsgTypeFilter): Promise<GeneralCallResult & {
        msgList: RawMessage[];
    }>;
    queryMsgsWithFilter(msgId: string, msgTime: string, param: QueryMsgsParams): Promise<GeneralCallResult & {
        msgList: RawMessage[];
    }>;
    ...
    setBuildMode(flag: number): unknown;
    ...
    fetchGetHitEmotionsByWord(args: unknown): Promise<unknown>;
    deleteAllRoamMsgs(arg1: number, arg2: string): unknown;
